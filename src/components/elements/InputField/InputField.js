...
export default (props) => {
    ...
    return (
        <div className="inputField">
            <span className={props.value.status === INPUT_STATUS.INVALID ? "input-span error" : 
                props.value.status === INPUT_STATUS.VALID ? "input-span success" : "input-span default"}>
                {input}
                {props.value.status === INPUT_STATUS.VALID ? 
                    <FontAwesomeIcon icon={faCheckCircle} className="faCheckCircle"/> : <div></div>}
                {props.value.status === INPUT_STATUS.INVALID ? 
                    <FontAwesomeIcon icon={faExclamationCircle} className="faExclamationCircle"/> : <div></div>}
            </span>
            {props.value.status === INPUT_STATUS.INVALID ? 
                    <small className="small-error">{props.value.errorMessage}</small> : <div></div>}
        </div>
    );
}